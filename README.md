...
Tags matching `v*` call the pinned foundation release workflow. The workflow rebuilds the exact tag, publishes the unsigned IPA and manifest to a GitHub Release, and verifies the released IPA hash. The current sample release contract is version `1.0.4`, build `5`; update both the workflow inputs and the app's bundle version/build together before creating the next tag.
